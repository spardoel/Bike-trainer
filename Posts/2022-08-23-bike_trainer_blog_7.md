...
## Previously...
Previously I talked a bit about how the database would be set up, so let's start there. In the previous post I described the tables being used. First there was the athlete table which holds the athlete id, name, weight and ftp.
```
...
Next let's talk about the class I created to interact with the database: the DatabaseHandler class.
...
class DatabaseHandler:
    def __init__(self):
        ...
        try:
            ...
            print(Error)
```
The connection is saved as a class property called conn. I did this so that I can easily access the connection from the other methods and I don't need to re-establish the connection inside every method. 
...
### get_athlete_power()
With those basics out of the way, let's look at the other methods. As mentioned earlier, all of the workouts are scaled according to the rider's functional threshold power (FTP). In short, FTP is a number (in Watts) representing the rider's current fitness level. The higher the FTP, the fitter the rider. If you want to learn more I suggest you google it, or ask a cyclist. Just like bros in the gym bragging about how much they can bench press, when provoked, a cyclist will enthusiastically tell you all about their FTP, what it means, and how their FTP is higher than their buddy's. All this to say, I needed to create a method to get the rider's FTP from the 'athletes' database. Here is what I came up with.
```
...
```
This method accepts an athlete id number and returns the athletes ftp. The method does the following. Using the connection that was created in the init method, the method creates a cursor. Then, using the cur.execute command it sends a SQL query to the database. The query selects the 'ftp' column from the 'athletes' table and returns the row where the 'athlete_id' matches the one provided. Then, using cur.fetchone, return the result of the SQL querry. 
...
### get_power_profile()
This method will return the power profile of a specific workout template. I wanted to make the method flexible enough to accept either the workout name or workout id as input parameters. A lot of the code in the method has to do with handling these two possible inputs. 
```
def get_power_profile(self, workout_name="", workout_id=[]):
        ...
        try:
            ...
            print(Error)           
```
The get_power_profile method accepts workout_name and workout_id as optional inputs. The first block of code checks whether there were any actual inputs. It checks the length of the input parameters, and if both are length 0, then it stop the method and returns an empty list. Assuming the method had at least one non null input, then it fetches data from the database. As with the previous 2 methods, I created a cursor, and executed a SQL query. The query in this case selects all of the template_id rows from the workout_templates table. Then, uing the cur.fetchall command, I accepted the result of the query as a tuple. Next, using a list comprehension I extracted the workout_id values and stored them in a class property called available_workout_ids. At this point in the code, I had an input (either the workout name or id) and I had a list of available workout ids to search through. If the given input was the workout id, then checking if the provided id is within the list of avilable ids is easy. But what if the workout name was provided? 
Let's take a closer look. I recoppied the relevant section of code below for reference. 
```
            # get the workout id value using the workout name
            if len(workout_name) > 0 and len(workout_id) == 0:
                self.workout_id = self.get_workout_id_from_name(workout_name)
...
```
The method accepts the rider_id and workout_name and inputs, these are needed to add a new row to the ride_logs table. The first thing the method does is converts the workout_name to the workout_id using get_workout_id_from_name(). 
Then it generates a cursor and executes a SQL command. This command inserts a new row into the ride_logs table. Note that to generate the date, I used date.today() from the datetime library. When I first tried to run this command, the result from date.today() was not accepted as a valid date in the SQL database. I then tried sending it as a string, which worked (and makes sense since dates are normally added as strings).  
After the command to add a row to the ride_logs table was sent, the conn.comit() command was used to commit the query the change to the database.
Once the row was added to the ride_logs table, and the corresponding ride_id value was generated, I needed to get that ride_id. I did that using another SQL query to select the ride_id where the ride_date was equal to today's date byt calling date.today() a second time. Wait. I suppose this could create a problem if the row was created just before midnight and the next query was executed a few minutes later (the next day)... I don't think that's likely. But to avoid the possibility altogether I updated the code so that the date is saved as a variable that is used in both queries - this way date.today() is only used once ensuring that the dates match. OK, potential but very unlikely crisis averted. Moving on.

Once the ride_id was obtained, it was used to generate the name of the ride table in the form of 'ride001' where 001 is the ride_id.
Then, another SQL execute() command was used to create the table with the newly generate name. 
Finally, the changes are comitted and the name of the table was returned. This name will be needed to update the table with the power data over the course of the workout.

## Wrap up
This post detailed the methods within the DatabaseHandler class. These methods were needed to fetch specific data from the database, to update tables, and to create new tables. The methods will be used by the other classes in the program and may need to be updated or changed, but for now this class is done :) On to the next one.
