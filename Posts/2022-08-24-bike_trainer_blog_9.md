...
In this post I want to talk about the main loop that runs within the start_workout() method of the WorkoutPlayer class. In the last post I set up the data callback to feed data into the loop as it becomes available. So let's jump into the loop. 

## The start_workout() method
The start workout method has a lot going on. It needs to get data from the trainer, update the ride_log table, tell the trainer to change the resistance when necessary, and keep track of multiple possible break conditions.
Since I set up the loop to run indefinitely (using while True:), the first thing I set up were stopping conditions. 
...
Here is the start_workout() method with the basic loop and the first stopping condition. 
```
...
```
 while True:
            ...
            if self.trainer.elapsed_time_sec > self.time_of_last_update:
                ...
                # update the time of last update and reset the timeout timer
                
                self.time_of_last_update = self.trainer.elapsed_time_sec
                self.timeout_timer = []
```
As you can see, if the trainer_elapsed_time_sec value is larger than time_of_last_update, then a new data packet has been received. In this case, time_of_last_update is set to the most recent elapsed time value and the timeout_timer is set to an empty list. 
Speaking of the timeout timer... 
If the elapsed_timed_sec is NOT larger than time_of_last_update, then no new data has been received. 
...
NO_NEW_DATA_TIMEOUT = 10
 ...
 if self.trainer.elapsed_time_sec > self.time_of_last_update:
                ...
                break
```
Alright, so I had 2 ways of exiting the infinite loop. Either the user pressed the escape key, or the trainer timed out without sending new data. There will also be a third way the loop exits (when the workout is completed normally) but that will be added later. 
...
```
As you can see, the method is pretty simple. It converts '00' to bytes then writes the result to the Fitness Machine Control Point characteristic.
...
Similar to the request_trainer_control() method, the set_trainer_resistance() method also used the control point characteristic on the trainer. Howerver, this method accepts the new desired power value as an input parameter and converts the value to a byte array before sending. The byte array also included the command code '5' which indicates a request to change the trainer resistance. Like this. 
```
...
```
Now, converting the new power to bytes then using indexes to access the first and second bytes probably isn't the most optimal solution. But to be honest, I was having issues using pack() from the struct library to generating the byte array format the trainer was looking for. In the end, I think the solution shown in the code above is reliable and easy to read so I am happy to leave it and move on.  
...
During the creation of the WorkoutPlayer class object, the init method calls the DatabaseHandler.get_power_profile() method and saves the output. This means that the workout template power profile is already available as a property within the start_workout() method. The main loop in the start_workout() method is responsible for logging the current user power to the batabase, checking the target power for the next second and sending the command to update the trainer resistance. 
At first I wanted to use the elapsed time received from the trainer as the index to get the next target power from the power_profile. But what if some delay were introduced? Or what if the workout is paused and when it resumes the elapsed time no longer coincided with the correct power value? After some thought, I decided I would use a specific counter to keep track of the progression through the power profile, and the elapsed time value will be used only to check if new data is available. 

### Defining the counter index and setting the new trainer target power
To start, I defined the counter as workout_index = [] before the main loop. Then, I created the following if statement to see if the power needed to be updated, and if so, send the command to change the trainer resitance, and update the current_power property.
```
...
As the main loop runs I wanted to update the database with the user power and cadence. For this I created the log_ride_power() method in the DatabaseHandler class.
...
async def start_workout(self):
        ...
        # Add new entry to the ride_logs table and create the ride table
